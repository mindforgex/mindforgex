import React, {
  ...
  const ChakraCarousel = ({ children, gap, limit = {base: 2, md: 2, xl: 4}, isShow = false, ...rest }) => {
    ...
    useEffect(() => {
      if (isBetweenBaseAndMd) {
        setItemWidth(sliderWidth / limit.base - gap);
        setMultiplier(0.65);
        setConstraint(limit.base);
      }
      if (isBetweenMdAndXl) {
        setItemWidth(sliderWidth / limit.md - gap);
        setMultiplier(0.5);
        setConstraint(limit.md);
      }
      if (isGreaterThanXL) {
        setItemWidth(sliderWidth / limit.xl - gap);
        setMultiplier(0.35);
        setConstraint(limit.xl);
      }
  ...
  const Slider = ({
    ...
    return (
      <>
        ...
        <Flex w={`${itemWidth}px`} mt={`${gap / 2}px`} mx="auto" display={isShow ? 'flex' : 'none'}>
          <Button
